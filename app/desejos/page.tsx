  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <nav className="bg-gray-800 border-b border-gray-700 px-6 py-3 flex gap-6">
        ...
        <a href="/desejos" className="text-white font-semibold border-b-2 border-indigo-500 pb-1">Desejos</a>
      ...
      <main className="max-w-6xl mx-auto px-6 py-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {jogosFiltrados.map(jogo => {
            ...
            return (
              <div key={jogo.id_jogo} className="bg-gray-800 rounded-2xl overflow-hidden border border-gray-700 hover:border-pink-500 transition group">
                {jogo.img_jogo && (
                  <img
                    src={jogo.img_jogo}
                    alt={jogo.nm_jogo}
                ...
                <div className="p-3">
                  ...
                  <div className="flex flex-wrap gap-1 mb-2">
                    {donos.map(d => (
                      <span key={d} className="text-xs bg-pink-900 text-pink-300 px-2 py-0.5 rounded-full">{d}</span>
                    ))}
                  </div>
        ...
        {membros.some(m => m.carregando) && (
          <div className="text-center text-gray-400 mt-8">Carregando listas...</div>
        )}
      </main>
    </div>
  )
